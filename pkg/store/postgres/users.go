...
func (s *PostgresStore) GetUserByEmail(ctx context.Context, email string) (*store.User, error) {
	var id string
	err := s.db.QueryRowContext(ctx, "SELECT id FROM users WHERE lower(email) = lower($1)", email).Scan(&id)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, store.ErrNotFound
		}
...
func (s *PostgresStore) ListUsers(ctx context.Context, filter store.UserFilter, opts store.ListOptions) (*store.ListResult[store.User], error) {
	...
	if filter.Search != "" {
		...
		conditions = append(conditions, fmt.Sprintf("(email ILIKE $%d OR display_name ILIKE $%d)", idx, idx+1))
	...
	if err := s.db.QueryRowContext(ctx,
		fmt.Sprintf("SELECT COUNT(*) FROM users %s", where), args...,
	...
	`, where, orderColumn, orderDir, idx, idx+1)
	...
	if len(users) > limit {
		...
		result.NextCursor = fmt.Sprintf("%d", offset+limit)
	}
