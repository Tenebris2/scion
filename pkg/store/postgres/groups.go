...
func (s *PostgresStore) GetGroup(ctx context.Context, id string) (*store.Group, error) {
	...
	`, id).Scan(
		&group.ID, &group.Name, &group.Slug, &group.Description,
		&group.GroupType, &projectID, &parentID,
		&labels, &annotations,
		&group.Created, &group.Updated, &group.CreatedBy, &group.OwnerID,
	)
	if err != nil {
...
func (s *PostgresStore) ListGroups(ctx context.Context, filter store.GroupFilter, opts store.ListOptions) (*store.ListResult[store.Group], error) {
	...
	if filter.GroupType != "" {
		conditions = append(conditions, fmt.Sprintf("group_type = $%d", idx))
	...
	if err := s.db.QueryRowContext(ctx,
		fmt.Sprintf("SELECT COUNT(*) FROM groups %s", where), args...,
...
func (s *PostgresStore) GetGroupMembers(ctx context.Context, groupID string) ([]store.GroupMember, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&member.GroupID, &member.MemberType, &member.MemberID, &member.Role, &member.AddedAt, &member.AddedBy); err != nil {
			return nil, err
		}
...
func (s *PostgresStore) GetEffectiveGroupsForAgent(_ context.Context, _ string) ([]string, error) {
	return nil, nil
}

func (s *PostgresStore) CheckDelegatedAccess(_ context.Context, _ string, _ *store.PolicyConditions) (bool, error) {
	return false, nil
}
...
func (s *PostgresStore) GetGroupsByIDs(ctx context.Context, ids []string) ([]store.Group, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&g.GroupType, &projectID, &parentID,
			&labels, &annotations,
