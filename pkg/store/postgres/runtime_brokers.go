...
const brokerCols = `id, name, slug, type, mode, version,
...
func scanBroker(row rowScanner) (*store.RuntimeBroker, error) {
	broker := &store.RuntimeBroker{}
	var capabilities, profiles, labels, annotations string
	var brokerType, brokerMode, harnesses, resources string
...
func (s *PostgresStore) GetRuntimeBroker(ctx context.Context, id string) (*store.RuntimeBroker, error) {
	row := s.db.QueryRowContext(ctx,
		"SELECT "+brokerCols+" FROM runtime_brokers WHERE id = $1", id)
	broker, err := scanBroker(row)
...
func (s *PostgresStore) GetRuntimeBrokerByName(ctx context.Context, name string) (*store.RuntimeBroker, error) {
	...
	err := s.db.QueryRowContext(ctx,
		"SELECT id FROM runtime_brokers WHERE LOWER(name) = LOWER($1)", name).Scan(&id)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, store.ErrNotFound
		}
...
func (s *PostgresStore) ListRuntimeBrokers(ctx context.Context, filter store.RuntimeBrokerFilter, opts store.ListOptions) (*store.ListResult[store.RuntimeBroker], error) {
	...
	if filter.ProjectID != "" {
		conditions = append(conditions, fmt.Sprintf("(id IN (SELECT broker_id FROM project_contributors WHERE project_id = $%d) OR auto_provide = 1)", idx))
	...
	if err := s.db.QueryRowContext(ctx,
		fmt.Sprintf("SELECT COUNT(*) FROM runtime_brokers %s", where), args...,
	...
	query := fmt.Sprintf("SELECT %s FROM runtime_brokers %s ORDER BY created_at DESC LIMIT $%d", brokerCols, where, idx)
...
func (s *PostgresStore) UpdateRuntimeBrokerHeartbeat(ctx context.Context, id string, status string) error {
	now := time.Now()
	result, err := s.db.ExecContext(ctx, `
		UPDATE runtime_brokers SET status = $1, last_heartbeat = $2, updated_at = $3 WHERE id = $4
	`, status, now, now, id)
	if err != nil {
		return err
	}
