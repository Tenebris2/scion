...
func (s *PostgresStore) ListMessages(ctx context.Context, filter store.MessageFilter, opts store.ListOptions) (*store.ListResult[store.Message], error) {
	...
	if filter.RecipientID != "" {
		conditions = append(conditions, fmt.Sprintf("recipient_id = $%d", idx))
		args = append(args, filter.RecipientID)
		idx++
	}
	if filter.SenderID != "" {
		conditions = append(conditions, fmt.Sprintf("sender_id = $%d", idx))
		args = append(args, filter.SenderID)
		idx++
	}
	if filter.ParticipantID != "" {
		conditions = append(conditions, fmt.Sprintf("(recipient_id = $%d OR sender_id = $%d)", idx, idx+1))
	...
	if err := s.db.QueryRowContext(ctx,
		fmt.Sprintf("SELECT COUNT(*) FROM messages %s", where), args...,
...
func (s *PostgresStore) MarkMessageRead(ctx context.Context, id string) error {
	result, err := s.db.ExecContext(ctx, "UPDATE messages SET read = 1 WHERE id = $1", id)
	if err != nil {
		return err
	}
...
func (s *PostgresStore) MarkAllMessagesRead(ctx context.Context, recipientID string) error {
	_, err := s.db.ExecContext(ctx, "UPDATE messages SET read = 1 WHERE recipient_id = $1", recipientID)
	return err
}
