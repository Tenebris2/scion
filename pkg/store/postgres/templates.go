...
func (s *PostgresStore) ListTemplates(ctx context.Context, filter store.TemplateFilter, opts store.ListOptions) (*store.ListResult[store.Template], error) {
	...
	if filter.Name != "" {
		conditions = append(conditions, fmt.Sprintf("(name = $%d OR slug = $%d)", idx, idx+1))
	...
	if filter.ScopeID != "" {
		conditions = append(conditions, fmt.Sprintf("(scope_id = $%d OR project_id = $%d)", idx, idx+1))
		args = append(args, filter.ScopeID, filter.ScopeID)
		idx += 2
	} else if filter.ProjectID != "" && filter.Scope == "" {
		conditions = append(conditions, fmt.Sprintf("(scope = 'global' OR (scope = 'project' AND (scope_id = $%d OR project_id = $%d)))", idx, idx+1))
	...
	if filter.Harness != "" {
		conditions = append(conditions, fmt.Sprintf("harness = $%d", idx))
	...
	if filter.Search != "" {
		conditions = append(conditions, fmt.Sprintf("(name LIKE $%d OR description LIKE $%d)", idx, idx+1))
	...
	if err := s.db.QueryRowContext(ctx,
		fmt.Sprintf("SELECT COUNT(*) FROM templates %s", where), args...,
...
func (s *PostgresStore) ListHarnessConfigs(ctx context.Context, filter store.HarnessConfigFilter, opts store.ListOptions) (*store.ListResult[store.HarnessConfig], error) {
	...
	if filter.ScopeID != "" {
		...
		conditions = append(conditions, fmt.Sprintf("(scope = 'global' OR (scope = 'project' AND scope_id = $%d))", idx))
	...
	var totalCount int
	if err := s.db.QueryRowContext(ctx,
		fmt.Sprintf("SELECT COUNT(*) FROM harness_configs %s", where), args...,
	).Scan(&totalCount); err != nil {
		return nil, err
	}
	...
	// Deduplicate by slug when querying by ProjectID without explicit Scope.
	if filter.ProjectID != "" && filter.Scope == "" {
		...
		for _, hc := range harnessConfigs {
			if i, exists := seen[hc.Slug]; exists {
				if hc.Scope == "project" && deduped[i].Scope == "global" {
					deduped[i] = hc
				}
			} else {
